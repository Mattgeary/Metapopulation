...
meta.s1_max_all <- data.frame(rep(list(run=numeric(years)), meta.runs)) 
...
meta.s1_max_all[,i] <- meta.1$maxent

print(i)
}

write.csv(meta.s1_pop, "Scenario_1_pop.csv",row.names=F)
write.csv(meta.s1_lek.size, "Scenario_1_lek_size.csv",row.names=F)
write.csv(meta.s1_n.leks, "Scenario_1_leks.csv",row.names=F)
write.csv(meta.s1_max, "Scenario_1_max.csv", row.names=F)
write.csv(meta.s1_max_all, "Scenario_1_max_all.csv", row.names=F)
