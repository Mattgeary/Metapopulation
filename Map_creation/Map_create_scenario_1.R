gc()
output.1 <- paste(getwd(), "Scenario_1", sep="/")
...
for(i in 1:iterations){
	if(cellStats(new.hab.1, sum) <= 20448.78) new.hab.1 <- f.Exp(new.hab.1, potential.pts, p.trans)
...
for(i in 1:iterations){
	if(cellStats(new.hab.2, sum) <= 13632.52) new.hab.2 <- f.Exp(new.hab.2, potential.pts, p.trans)
	else new.hab.2 <- new.hab.2
    }

dir.create(paste(getwd(), "/Scenario_1/run_", j, sep=""))
...
writeRaster(pred, filename=(paste(getwd(), "Scenario_1", paste("pred_map", j, ".grd", sep= ""), sep="/")), overwrite=T)
rm(env.l.new)
gc()
