...
map <- raster(paste("s2_large_map_",sm, ".asc", sep=""), proj4=BNG)
...
write.csv(meta.s2_pop, "Scenario_2_pop_small.csv", row.names=F)
write.csv(meta.s2_lek.size, "Scenario_2_lek_size_small.csv", row.names=F)
write.csv(meta.s2_n.leks, "Scenario_2_leks_small.csv", row.names=F)
save.image("Scenario_2_small.RData")
